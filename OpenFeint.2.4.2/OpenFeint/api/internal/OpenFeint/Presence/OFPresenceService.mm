...
-(void)sendHttpRequest:(NSString *)theRequest
{
	...
	if (contentType == nil) {
        ...
        NSURLConnection * theUrlConnection = [httpRequest objectAtIndex:0];
		[theUrlConnection scheduleInRunLoop:[NSRunLoop currentRunLoop] forMode:NSDefaultRunLoopMode];
		[theUrlConnection start];
		[httpRequests removeObjectForKey:theRequest];
	}
